...
function TradeQueryGeneratorClass:GetProgressTotal()
	...
	if self.calcContext.options.includeSynthesis then
		total = total + self:CountPendingModWeights(self.modData["Synthesis"], seen)
	end
	return total
end
...
function TradeQueryGeneratorClass:StartQuery(slot, options)
	...
	self.calcContext.progress = {
		...
		slotLabel = self:GetProgressSlotLabel(context, slot),
		current = 0,
...
function TradeQueryGeneratorClass:FinishQuery()
	...
	local function buildQueryTable(baseOverride)
		local queryFilters = copyTable(self.calcContext.special.queryFilters or { type_filters = { filters = { } } }, true)
		...
		if self.calcContext.itemCategoryQueryStr then
			queryFilters.type_filters.filters.category = queryFilters.type_filters.filters.category or { option = self.calcContext.itemCategoryQueryStr }
		end
...
function TradeQueryGeneratorClass:RequestQuery(slot, context, statWeights, callback, skipPopup)
	...
	controls.includeCorrupted = new("CheckBoxControl", {"TOP",nil,"TOP"}, {-88, 30, 18}, "Corrupted Mods:", function(state) end)
	...
	elseif slot and not isAbyssalJewelSlot and context.slotTbl.slotName ~= "Watcher's Eye" then
		...
		controls.influence1.selIndex = self.lastInfluence1 or INFLUENCE_IGNORE_INDEX
		...
		controls.influence2.selIndex = self.lastInfluence2 or INFLUENCE_IGNORE_INDEX
	...
	local function executeQuery()
		...
		if controls.selectedBase then
			...
			elseif autoBaseDefenceProfile then
				...
				options.autoBaseSearchCount = popupAutoBaseSearchCount
				self.lastAutoBaseSearchCount = popupAutoBaseSearchCount
			else
