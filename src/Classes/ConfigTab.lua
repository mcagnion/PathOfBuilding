...
local configEnemyConditionHelper = LoadModule("Modules/ConfigEnemyConditionHelper")
			...
			if varData.ifMinionCond then
				...
				if varData.ifEnemyCond then
					...
					t_insert(tooltipFuncs, listOrSingleIfTooltip(varData.ifEnemyCond, function(ifOption)
						...
						local sourceTooltip = configEnemyConditionHelper.formatConditionSourcesTooltip(
							self.build,
							mods
						)
						local chanceTooltip = configEnemyConditionHelper.formatConditionChanceTooltip(
							self.build,
							ifOption
						)
						local recommendationTooltip = configEnemyConditionHelper.formatConditionRecommendationTooltip(
							self.build,
							getActiveConfigInput(),
							varData.ifEnemyCond,
							varData.var
						)
						local tooltipPrefix = configEnemyConditionHelper.joinTooltipLines(
							sourceTooltip,
							chanceTooltip,
						...
						for _, mod in ipairs(mods) do
							out = (out and out.."\n" or "")
								...
								.. configEnemyConditionHelper.formatConditionSource(self.build, mod.source)
						end
				...
				local extraEnemyConditionHints =
					configEnemyConditionHelper.getEnemyConditionHints(varData)
				if extraEnemyConditionHints and not varData.ifEnemyCond then
					t_insert(tooltipFuncs, function()
						local mods = configEnemyConditionHelper.collectEnemyConditionMods(
							self.build,
						...
						return configEnemyConditionHelper.joinTooltipLines(tooltipPrefix, out)
					end)
				end
					...
					control.borderFunc = function()
						...
						if cur ~= nil and cur ~= def then
							...
							if shown and varData.type == "check" and recommendationIfEnemyCond then
								local recommendation = configEnemyConditionHelper.getEnemyConditionRecommendationData(
									self.build,
									getActiveConfigInput(),
