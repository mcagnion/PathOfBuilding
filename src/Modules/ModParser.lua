...
local modTagList = {
	...
	["if there are no (%a+) modifiers on other equipped items"] = function(_, conditionSubstring)
		if conditionSubstring == "defence" then
			-- "Defence Modifiers" cannot be reliably auto-detected from mod text (e.g. "% increased Armour"
			-- doesn't contain "defence"). Use a manual config condition instead.
			return { tag = { type = "Condition", var = "NoDefenceModifiersOnEquipment" } }
		end
		return {tag = { type = "ItemCondition", searchCond = conditionSubstring, itemSlot = "{SlotName}", allSlots = true, excludeSelf = true, neg = true }}
	end,
