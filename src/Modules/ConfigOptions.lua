...
return {
	...
	{ var = "EnableEHPRecoveryCalcs", type = "check", label = "Enable recovery between hits:", tooltip = "Enables the experimental recovery between hits calculations and UI.\nDisabled by default because it can slow down calculations." },
	{ var = "conditionNoDefenceModifiersOnEquipment", type = "check", label = "No Defence Modifiers on Equipment?", ifCond = "NoDefenceModifiersOnEquipment", tooltip = "Enable when no other equipped item has a Defence Modifier (Armour, Evasion, Energy Shield, Ward).\nRequired for 'increased Global Defences if there are no Defence Modifiers on other Equipped Items' to apply.", apply = function(val, modList, enemyModList)
		if val then
			modList:NewMod("Condition:NoDefenceModifiersOnEquipment", "FLAG", true, "Config")
		end
	end },
...
Huge sets the radius to 11.
	...
	{ var = "TotemsSummoned", type = "count", label = "# of Summoned Totems (if not maximum):", ifStat = "TotemsSummoned", ifSkillFlag = "totem", implyCond = "HaveTotem", tooltip = "This also implies that you have a Totem summoned.\nThis will affect all 'per Summoned Totem' modifiers, even for non-Totem skills.", apply = function(val, modList, enemyModList)
	...
	{ var = "conditionEnemyIntimidated", type = "check", label = "Is the enemy Intimidated?", ifEnemyCond = "Intimidated", tooltip = "Intimidated enemies take 10% increased Attack Damage.", apply = function(val, modList, enemyModList)
		enemyModList:NewMod("Condition:Intimidated", "FLAG", true, "Config", { type = "Condition", var = "Effective" })
	end },
	{ var = "conditionEnemyCrushed", type = "check", label = "Is the enemy Crushed?", ifEnemyCond = "Crushed", tooltip = "Crushed enemies have 15% reduced Physical Damage Reduction.", apply = function(val, modList, enemyModList)
	...
	{ var = "conditionEnemyUnnerved", type = "check", label = "Is the enemy Unnerved?", ifEnemyCond = "Unnerved", tooltip = "Unnerved enemies take 10% increased Spell Damage.", apply = function(val, modList, enemyModList)
