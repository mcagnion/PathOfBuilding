...
describe("TestRadiusJewelStatDiff", function()
	...
	it("AddItemTooltip emits an equip-comparison block for an unequipped Timeless jewel", function()
		local spec, socketNode = setupAllocatedSocket()
		...
		local targetNode = nodesInRadius[1]
		local origTreeNode = spec.tree.nodes[targetNode.id]
		local conqueredCopy = {
			id = origTreeNode.id,
			type = origTreeNode.type,
			dn = origTreeNode.dn,
			modKey = origTreeNode.modKey or "",
			conqueredBy = item.jewelData.conqueredBy,
			modList = new("ModList"),
		}
		conqueredCopy.modList:NewMod("Life", "BASE", 100, "Timeless Jewel")

		spec.ComputeConqueredNodes = function(self, socketId, conqueredBy, radiusIdx)
			if socketId == socketNode.id then
				return { [targetNode.id] = conqueredCopy }
			end
			return { }
		end

		local stubCalled = false
		local origStub = spec.ComputeConqueredNodes
		spec.ComputeConqueredNodes = function(self, socketId, conqueredBy, radiusIdx)
			stubCalled = true
			return origStub(self, socketId, conqueredBy, radiusIdx)
		end

		local tooltip = new("Tooltip")
		build.itemsTab:AddItemTooltip(tooltip, item, slot)

		assert.is_true(stubCalled,
			"AddItemTooltip should call spec:ComputeConqueredNodes for an unequipped timeless jewel")
		assert.is_true(tooltipContains(tooltip, "Equipping this item"),
			"tooltip should contain an 'Equipping this item' comparison header")
	end)
