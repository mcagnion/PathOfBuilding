...
describe("TestRadiusJewelStatDiff", function()
	...
	it("AddNodeTooltip shows conquered node delta for an allocated conquered node", function()
		local spec, socketNode = setupAllocatedSocket()
		...
		-- Minimal PassiveTreeView stub with showStatDifferences enabled
		local viewer = { showStatDifferences = true, compareSpec = nil, tracePath = nil }
		local PassiveTreeViewClass = getmetatable(build.treeTab.viewer)
		setmetatable(viewer, PassiveTreeViewClass)

		local tooltip = new("Tooltip")
		viewer:AddNodeTooltip(tooltip, nodesInRadius[1], build)

		assert.is_true(tooltipContains(tooltip, "Timeless jewel on this node gives you"),
			"tooltip should contain a 'Timeless jewel on this node gives you' header")
	end)
